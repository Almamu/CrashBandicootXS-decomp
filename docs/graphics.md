...
`baserom.gba` and the current split `graphics/unknown/*/*.frames`
sources, never writes to them). Purpose: once the actor/entity C code
behind this system is reversed, this is the *data* half ready to go -
real ROM addresses, per-record keyframe->frame mappings, and direct
`asset_file`/`frame_index` references into the already-extracted PNGs -
so whoever writes the matching structs doesn't have to re-derive any of
this from raw hex again.
...
used `table_B` index resolved to either `{asset_file, frame_index,
w_tiles, h_tiles}` for `pool_offset` records or `{rom_address, w_tiles,
h_tiles}` for `absolute_rom` ones). Confirmed content from this and the
