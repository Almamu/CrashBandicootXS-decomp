...
just graphics) - the workflow, gotchas, per-function notes, and cleanup
passes for turning hand-disassembled `asm/code_3_*.s` blocks into real,
compiling C. See [graphics.md](./graphics.md) for the separate, unrelated
topic of graphics/sprite asset extraction, and [audio.md](./audio.md) for
the sound engine.
...
**The per-function loop.** Every function that goes from `asm/code_3_*.s`
into real C follows this same loop, in order, end to end, every time -
this includes the cleanup step (step 7), which in earlier sessions
happened as an occasional separate pass over a batch of already-matched
functions (see the two "Cleanup pass" entries near the end of this doc
for what that used to look like) but is now part of matching each single
function, not something deferred and done later in bulk:
...
3. Iterate on the C until it's byte-exact - register-variable pins,
   inline-asm anchors, and the alignment-padding fix below are all fair
   game (see `matching_decomp_register_pinning`/
   `matching_decomp_alignment_fix` memory for the established
   techniques). If a genuine gap resists every technique tried, park it
   under the `NON_MATCHING` build toggle instead of leaving raw asm or
   giving up on the C reconstruction entirely - see
   `matching_decomp_non_matching_toggle` memory and the "Parked, not
   matched" entries below for the pattern and when to stop iterating.
4. Once it matches (or is deliberately parked), cut that function's
   ...
   contiguous ROM region, not one per "topic"** - see
   `GetAnimFrameBaseOffset`'s entry below for why a function whose real
   address isn't adjacent to an existing matched file's functions needs
   its own new `.c` file instead.
...
7. **Cleanup pass, on this function alone, right now - do not defer it
   ...
   - **Rebuild and re-run `make compare` (and `make NON_MATCHING=1
     ...
     below) - any such regression gets reverted immediately, not left in
     on the theory that it's "close enough."
8. Update this doc's per-function log (a short paragraph like the ones
   below is enough; a subtler fix may deserve its own explained bullet
   like the "Cleanup pass" entries) and `README.md`'s status list, then
   commit.
