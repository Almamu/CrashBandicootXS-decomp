...
- **`sub_802A69C`/`sub_802A6B0`/`sub_802A6C4`/`sub_802A6D8`**
  (`src/graphics/actor_part39.c`) - four trivial forwarders, the same
...
- **`sub_802A6EC`** (`src/graphics/actor_part39.c`) - passes its own
  `self` argument through to `sub_803AD7C`, alongside a function pointer
  read from `gUnknown_03001418`'s own `+0x24` field.
- **`InitActorPart`** (`src/graphics/actor_part39.c`) - the constructor.
...
- **`sub_802A7B8`** (`src/graphics/actor_part39.c`) - the movement-
...
- **`sub_802A980`** (`src/graphics/actor_part45.c`) - the same
  movement-threshold recompute as `sub_802A7B8`, with no trampoline-
  fire/frame-advance tail.
- **`sub_802A9D4`** (`src/graphics/actor_part45.c`) - trivial getter:
  the first byte of `self`'s part-table pointer.
- **`sub_802A9DC`** (`src/graphics/actor_part45.c`) - the state/table-
  index/anim-frame reset idiom already documented for the boss
  cluster's `sub_8030530`/`sub_8030C98`.
- **`sub_802AA00`/`sub_802AA04`/`sub_802AA08`** (`src/graphics/actor_part45.c`)
  - trivial `self+0x24`/`0x20`/`0x1c` getters.
- **`sub_802AA4C`** (`src/graphics/actor_part41.c`) - trivial `self+0x2c`
  byte getter.
- **`sub_802AA54`** (`src/graphics/actor_part41.c`) - teardown: marks
  ...
  when requested.
- **`sub_802AA80`/`sub_802AAB4`/`sub_802AAFC`** (`src/graphics/actor_part41.c`)
...
- **`sub_802AB08`/`sub_802AB34`** (`src/graphics/actor_part41.c`) - the
  palette-cycle cursor/bound save/restore pair.
- **`sub_802ABC8`/`sub_802ABFC`** (`src/graphics/actor_part43.c`) - the
  palette-cycle cluster's remaining seed/arm-disarm pair, non-adjacent to
  `actor_part41.c` since the parked `sub_802AB58` sits raw between them.

## Parked (3 of 25 functions, `NON_MATCHING`)

- **`UpdateAnimatedActorPart`** (`asm/code_3_2_20_8b7c_a88c.s`, C in
  `src/graphics/actor_part44.c`) - the OAM draw/scale routine: computes
...
that file into `actor_part41.c`/`actor_part43.c` - shifting every
