...
UNITS = [
    ...
    (0x08002868, None, "system"),  # sub_8002868/sub_8002938 - EEPROM load/save block loops for the settings record (built on src/system/timer_util.c's EEPROM primitives) - NAKED transcriptions, byte-correct but not real decompiled C, tracked as parked - issue #4, see docs/matching/issue-4-sio-settings-sync.md
    ...
    (0x0800A0FC, None, "graphics"),  # sub_800A0FC through sub_800A420 (part-object update/collision dispatchers calling still-unexamined sub_800A178, and sub_8009BE0 - now understood and NAKED-parked, see docs/matching/naked-spatial-grid-tail.md) - still raw. GitHub issue #9 (chunk generator labeled this "game_loop"; corrected to "graphics" to match the surrounding actor_part-family entries, see docs/matching/issue-9-0x08007634-actor.md) - docs/rom_map.md "Two big unnamed systems"
    (0x0800A528, "src/graphics/actor_part47.o", "graphics"),  # sub_800A528/sub_800A590 (issue #9): a moving-platform "ride along" hookup - moves self+4 (Y position) by the delta between this call's and the last call's position record (via a self->table+0x10/0x14 trampoline), interpreted differently per self+0x68 state (8 or 4). sub_800A528 is the same body with an extra unconditional sub_8009FB0(self) call first. PARKED (NON_MATCHING): every load/store/branch confirmed correct, self/rec register roles (r4/r3) match the ROM, but the ROM's inner scratch-register use (a genuine 5th register, r5, just to hold an offset immediate) isn't reproduced by any C phrasing tried - see docs/matching/issue-9-0x08007634-actor.md. Real bytes in the new asm/code_3_2_11_a528.s (asm/code_3_2_11.o trimmed to end right before this pair). New object, inserted between the trimmed asm/code_3_2_11.o and actor_part14.o
    ...
    (0x0800A734, None, "graphics"),  # sub_800A734 (issue #9): part-object velocity/state reset+constructor - hooks up a child object at self+0xb0 via sub_800815C. PARKED (NON_MATCHING): field writes confirmed correct one-for-one, but the ROM's running-pointer address-increment style across a long, non-uniform stretch of field writes wasn't reproduced by any C phrasing tried - see docs/matching/issue-9-0x08007634-actor.md. Real bytes in asm/code_3_2_16_a734.s (now holding only this function, right before the matched sub_800A810 below)
    (0x0800A810, "src/graphics/actor_part48.o", "graphics"),  # sub_800A810 (issue #9): dispatches a sub-state byte (self+0x88) to one of three teardown helpers, each called with the same self+0x44 record argument sub_800A730 already established. Matched: self pinned to r3 across all three bl calls, the two AND-mask field clears built via the register-pinned negative-constant idiom (register s32 mask asm("r0") = -0x21/-9, forcing the ROM's runtime mov+neg pair), the self+0x68-to-self+0x28 field address as a single decremented u8 *p cursor, and the dispatch itself rewritten with explicit gotos plus a second register-pinned copy of the state byte (asm("r1")) for the inner comparisons - see docs/matching/issue-9-0x08007634-actor.md. New object, inserted between the trimmed asm/code_3_2_16_a734.o (now ending right after sub_800A734) and the trimmed asm/code_3_2_16.o (still starting at sub_800A884)
    (0x0800A884, None, "graphics"),  # sub_800A884 through sub_800AFF4 (calls still-unexamined sub_800815C-family/sub_8026628/sub_8026BC0/etc. and dispatches through several large jump tables) - still raw. GitHub issue #9 - docs/rom_map.md "Two big unnamed systems"
    (0x0800B270, "src/graphics/actor_part49.o", "graphics"),  # sub_800B270 (issue #9): per-frame velocity integrator - moves self+0x60/self+0x64 (X/Y velocity) toward self+0x50/self+0x5c (target velocity) by self+0x4c/self+0x58 (step), derives a self+0x24 direction-flag byte, snapshots position into self+0x6c/self+0x70, applies velocity to self+0/self+4, and records the Y velocity into an unlabeled global at 0x0300129C. PARKED (NON_MATCHING): every instruction up through the position-update store matches (needed self/v/target/result pinned to r2/r1/r3/r0, the flags pointer to r1, forced fresh reloads matching the ROM's redundant re-reads of self->x/self->y); the final 14-instruction 0x0300129C block has the global's address/value swapped between r0/r2 relative to the ROM, not reproduced by any register pin combination tried - see docs/matching/issue-9-0x08007634-actor.md. Real bytes in the new asm/code_3_2_16_b270.s. New object, inserted between the trimmed asm/code_3_2_16.o and actor_part15.o
    ...
    (0x0800D18C, None, "game_loop"),  # GitHub issue #12: sub_0800D18C (the ~1960B collision-response commit function docs/rom_map.md reads in detail - 6-case jump table dispatching to sub_800F2BC/sub_800F368/sub_800E7A8/sub_800EEF0/sub_800E6B0, a 5-slot recent-collision ring buffer inside gUnknown_030012D8, ends by calling sub_8010D54) and sub_800E08C (1032B, a further state/jump-table dispatcher in the same subsystem) - both left raw, out of scope for this pass; real bytes in asm/code_3_2_17_d18c.s - see docs/matching/issue-12-physics-collision.md
    ...
    (0x0800E560, None, "game_loop"),  # GitHub issue #12: sub_800E560 onward through sub_800F990 (the rest of this chunk's 25-function list) - the jump-table collision-response handlers (sub_800E620/sub_800E6B0/sub_800E7A8/sub_800E888/sub_800EAFC/sub_800ED08/sub_800EDBC/sub_800EEF0/sub_800F06C/sub_800F1B8/sub_800F258/sub_800F2BC/sub_800F368/sub_800F4F4/sub_800F5B8/sub_800F6B8/sub_800F798/sub_800F8E0/sub_800F990) sub_0800D18C's jump table dispatches into; left raw, out of scope for this pass - see docs/matching/issue-12-physics-collision.md
    (0x0800FC70, None, "game_loop"),  # GitHub issue #13: sub_800FC70/sub_800FDC8 - left raw (untouched), out of scope for this pass; real bytes in asm/code_3_2_17_e560_fc70.s. Recategorized graphics->game_loop, same reasoning as issue #12's 0x0800D040 entry above (docs/rom_map.md's confirmed shared physics/collision subsystem, not per-entity behavior) - see docs/matching/issue-13-graphics-fc70.md
    ...
    (0x0800FF0C, None, "game_loop"),  # GitHub issue #13: sub_800FF0C/sub_8010480/sub_80104E4/sub_8010674 - left raw, out of scope for this pass; real bytes in asm/code_3_2_17_e560_ff0c.s - see docs/matching/issue-13-graphics-fc70.md
    (0x080106DC, "src/system/game_loop23.o", "game_loop"),  # GitHub issue #13: sub_80106DC/sub_8010708/sub_801070C/sub_8010710/sub_8010714/sub_8010718 (matched) - viewport-box refresh plus the "get next"/"get prev"/"set next"/"set prev" neighbor-list accessor quartet docs/rom_map.md ties to sub_0800D18C - see docs/matching/issue-13-graphics-fc70.md
    (0x0801071C, None, "game_loop"),  # GitHub issue #13: sub_801071C/sub_801075C/sub_8010784/sub_80107C4 (the latter two Bresenham-line-style helpers) - left raw, out of scope for this pass; real bytes in asm/code_3_2_17_e560_1071c.s - see docs/matching/issue-13-graphics-fc70.md
    ...
    (0x0801089C, None, "game_loop"),  # GitHub issue #13: sub_801089C - left raw, out of scope for this pass; real bytes in asm/code_3_2_17_e560_1089c.s - see docs/matching/issue-13-graphics-fc70.md
    ...
    (0x08010914, None, "game_loop"),  # GitHub issue #13: sub_8010914/sub_801095C/sub_80109A4 - left raw, out of scope for this pass; real bytes in asm/code_3_2_17_e560_10914.s - see docs/matching/issue-13-graphics-fc70.md
    ...
    (0x08010A0C, None, "graphics"),  # start of the next still-unexamined chunk past issue #13's range; category left as the pre-existing placeholder pending its own examination, same convention as the former 0x0800FC70 entry above
    ...
    (0x08011BD4, None, "actor"),  # GitHub issue #16: sub_8011BD4 onward through sub_8012D24 (the chunk's remaining 10 functions) - sub_8011BD4 itself is docs/rom_map.md's documented ~1420B, 25-case/7-case nested jump-table companion state machine to sub_8016288 (still raw, type-`0x1d` player-control family), and the rest (sub_8012160/8238/22CC/2420/2694/283C/2A7C/2AF4/2D24) are further members of the 42-slot action-dispatch-table family (gStaticData_0816BF20) reading/writing a still-unnamed "child object" struct (self+0xc/+0x10/+0x18 sub-record pointers) that rom_map.md itself says isn't understood with byte-exact precision yet; left raw, out of scope for this pass - see docs/matching/issue-16-actor-11b0c.md
    (0x08012FBC, None, "graphics"),  # start of the next still-unexamined chunk past issue #16's range (sub_8012FBC/sub_8013228/etc.); category left as the pre-existing placeholder pending its own examination, same convention as the 0x0800FC70 entry above
    (0x0801426C, "src/graphics/actor_part18.o", "graphics"),  # sub_801426C/sub_80142B0 - two entries of the 42-slot action dispatch table (gStaticData_0816BF20, docs/rom_map.md) sharing a state/flag/table-index trio (+0x27/+0x28/+0x29/+0x2f/+0x30/+0x31/+0x32) and the usual base+offset+fn-pointer trampoline pair via self+0xc/self+0x10; matched. Non-adjacent to actor_part17.o since a large raw span (sub_800B8DC onward, including the still-unexamined sub_8012FBC/sub_8013228/sub_80134B8/sub_8013994 neighbors of this same table) sits between them
    ...
    (0x0801574C, "src/graphics/actor_part38d.o", "actor"),  # nullsub_17-sub_8015780 (issue #18): two nullsubs, a couple of tail-call wrappers, and the shared sub_8015780 trampoline-pair-plus-sentinel-store helper called by actor_part18.o's sub_801426C/sub_80142B0; matched
    (0x080157C4, None, "actor"),  # sub_80157C4 (issue #18): player's +0x100-flag-gated mode remapper tail-calling sub_800B86C; NAKED transcription (byte-correct, not real decompiled C), tracked as parked - see docs/matching/issue-18-0x08014f8c-actor.md
    ...
    (0x080159F8, None, "actor"),  # sub_80159F8/sub_8015C6C/sub_8015DF8 (issue #19): three large jump-table state-machine dispatchers in the same "self" object family; left raw, out of scope for this pass - real bytes in the new asm/code_3_2_17_159f8.s
    (0x08015FDC, "src/graphics/actor_part57b.o", "actor"),  # sub_8015FDC (issue #19): a player-velocity-relative record writer; matched. Non-adjacent to actor_part57.o since the left-raw sub_80159F8/sub_8015C6C/sub_8015DF8 sit between them. sub_8016046 right after it is disassembler-rendered zero-fill padding, not real code - reproduced by this unit's own trailing alignment, no separate symbol
    ...
    (0x08017AB0, None, "actor"),  # sub_8017AB0 (issue #22) - a large (~500-instruction) player-vs-camera-viewport state dispatcher; left raw, out of scope for this pass - real bytes stay in the new asm/code_3_2_17_17ab0.s
    (0x08017ECC, "src/graphics/actor_part27b.o", "actor"),  # sub_8017ECC-sub_8017FE8 (issue #22): self+4 double-pointer-chain record lookups feeding the gStaticData_0816C2D8 per-vector-component trampoline table, plus more self+0xc table-pointer accessors; matched. Non-adjacent to actor_part27.o since the raw sub_8017AB0 sits between them
    ...
    (0x0801E688, None, "graphics_loading"),  # sub_801E688/sub_801E788 (issue #30) - LoadGraphicsPackage's tile-cell-selection and viewport-centering helpers (docs/rom_map.md); left raw, out of scope for this pass - real bytes stay in the new asm/code_3_2_17_1e644.s
    ...
    (0x0801E950, None, "graphics_loading"),  # GitHub issue #30: sub_801E950 - packs a second bitfield into the same scratch buffer byte sub_801E8F8 writes; NAKED transcription (byte-correct, not real decompiled C), tracked as parked - see docs/matching/issue-30-graphics-loading.md
    ...
    (0x0801E990, None, "graphics_loading"),  # remainder of the LoadGraphicsPackage cluster (issue #30) - sound-trigger dispatch plus the "trigger effect"/text-popup-spawner families; left raw, out of scope for this pass - real bytes stay in asm/code_3_2_17_1e990.s
    ...
    (0x0801FEEC, None, "graphics_loading"),  # most of the rest of the chunk 31 range - still raw, real bytes in asm/code_3_2_17_1feec.s (now shorter - trigger_effect.o's span, below, was carved out of the middle of it)
    ...
    (0x08021280, None, "graphics_loading"),  # rest of the chunk 31 range past the twin family - still raw, real bytes in the new asm/code_3_2_17_21280.s
    ...
    (0x08021D04, None, "graphics_loading"),  # GitHub issue #33: sub_8021D04 - a sub_800FF0C trampoline plus a per-record flags-byte lookup; NAKED transcription (byte-correct, not real decompiled C), tracked as parked - see docs/matching/issue-33-0x08021bfc-graphics-loading.md
    ...
    (0x080234E8, "src/system/game_loop10.o", "game_loop"),  # GitHub issue #37: sub_80234E8-sub_80236AC (14 fns): self+0x1b8/0x1c0 camera-position setters, self+0xa4/0xa6 busy-flag setters, level-transition/checkpoint snapshot helpers (sub_8023548/sub_802356C/sub_80235E4), the sub_8022468 mode-trampoline family, and the snapshot bitfield unpacker; matched. Also incl. parked sub_80236EC (NON_MATCHING C reconstruction, real bytes in asm/code_3_2_17_236ec.s) - the bitfield unpacker's inverse - see docs/matching/issue-37-game-loop-234e8.md
    (0x08023738, "src/system/game_loop11.o", "game_loop"),  # GitHub issue #37: sub_8023738 - lazily allocates and returns gUnknown_03000828 (0x1cc bytes); matched. Its own object since the still-raw sub_802375C/sub_8023A1C pair sits on both sides (asm/code_3_2_17_236ec.s before it, asm/code_3_2_17_2375c.s after)
    (0x0802375C, None, "game_loop"),  # GitHub issue #37: sub_802375C/sub_8023A1C - two large level-start/post-level-end state-machine functions (gStaticData_0816C8xx tables, several still-raw callees); left untouched, not yet confidently understood branch-by-branch - real bytes in the new asm/code_3_2_17_2375c.s - see docs/matching/issue-37-game-loop-234e8.md
    (0x0802400C, "src/system/game_loop8.o", "game_loop"),  # GitHub issue #37: sub_802400C (matched) + sub_80240E4 (parked, NON_MATCHING, real bytes in asm/code_3_2_17_240e4.s) - the DMA3/VRAM refresh pass and the REG_BLDCNT/REG_BLDALPHA shadow-word rebuild - see docs/matching/issue-37-game-loop-234e8.md
    ...
    (0x08024344, None, "game_loop"),  # sub_8024344 - left in raw asm/code_3_2_17_24344.s; semantics fully traced (medal-table item-list nonzero scan) but resists byte-exact match, apparently an ip/r12 register pin for the flagIdx parameter this reconstruction doesn't reproduce - see docs/matching/issue-38-medal-results-tally.md
    (0x080243E0, "src/system/game_loop18.o", "game_loop"),  # GitHub issue #38: sub_80243E0 through sub_802455C (13 functions) - medal-table entry/item-list field accessors and the sub_8024344 constant wrappers; matched - see docs/matching/issue-38-medal-results-tally.md
    (0x08024590, None, "game_loop"),  # sub_8024590/sub_8024640/sub_80246D8/sub_8024708 - left in raw asm/code_3_2_17_24590.s; semantics traced (sound-channel handle lifecycle, item-list driver, and a VRAM-bank-toggling streamer+DISPCNT writer) but resist byte-exact match on this reconstruction's register allocation - see docs/matching/issue-38-medal-results-tally.md
    ...
    (0x08024790, None, "game_loop"),  # sub_8024790 - left in raw asm/code_3_2_17_24790.s; same sound-channel-lifecycle family as the sub_8024590 span above - see docs/matching/issue-38-medal-results-tally.md
    ...
    (0x08024810, None, "game_loop"),  # remainder of the UpdateGameFrame-MainLoop cluster, still raw - real bytes in asm/code_3_2_17_24810.s
    ...
    (0x080255D4, None, "game_loop"),  # sub_80255D4 - DMA-writes to OBJ palette RAM and a BG window register, then walks a small count-prefixed gUnknown_030012E4 list (docs/rom_map.md: "a per-frame visible-object/window list processor"); then sub_8025894 (a group/item list counter, parked NON_MATCHING, real bytes stay here in the now-truncated asm/code_3_2_17_255d4.s) - both left raw, GitHub issue #40/#41
    (0x08025944, "src/system/game_loop12.o", "game_loop"),  # GitHub issue #41: sub_8025944/sub_8025968/sub_802599C - the first two of three overlapping bit-grid accessors at self+8/self+0x208/self+0x308; matched. Also incl. parked sub_8025894 (NON_MATCHING C reconstruction widens this unit past its own real 0x08025944 start, per the "parked function" convention - see e.g. sub_80073DC above) - stays raw, real bytes in asm/code_3_2_17_255d4.s - see docs/matching/issue-41-game-loop-25894.md
    (0x080259D4, None, "game_loop"),  # GitHub issue #41: sub_80259D4 - sets a bit in both the self+0x208 and self+0x308 bit-grids at once; left raw, real bytes in asm/code_3_2_17_259d4.s - see docs/matching/issue-41-game-loop-25894.md
    (0x08025A0C, "src/system/game_loop13.o", "game_loop"),  # GitHub issue #41: sub_8025A0C (third bit-grid setter)/sub_8025A3C (Q8-to-int store)/sub_8025A44 (conditional sub_8026ED0 forward)/sub_8025A5C (zero two Q8 words); matched. Also incl. parked sub_80259D4 (NON_MATCHING C reconstruction widens this unit past its own real 0x08025A0C start) - stays raw, real bytes in asm/code_3_2_17_259d4.s - see docs/matching/issue-41-game-loop-25894.md
    (0x08025A64, None, "game_loop"),  # GitHub issue #41: sub_8025A64/sub_8025B0C/sub_8025BAC/sub_8025CA4 - four part-object spawn helpers (gUnknown_030012D0-table-indexed, sub_8009ED0/sub_8011114/sub_801173C-family constructors); left raw, real bytes in asm/code_3_2_17_25a64.s - see docs/matching/issue-41-game-loop-25894.md
    (0x08025D28, "src/system/game_loop14.o", "game_loop"),  # GitHub issue #41: sub_8025D28 (table-indexed function-pointer dispatch via the interworking-trampoline convention)/sub_8025D4C (store two Q8 words)/sub_8025D54 (conditional sub_8026ED0 forward, dup of sub_8025A44)/sub_8025D6C (zero two Q8 words, dup of sub_8025A5C); matched. Also incl. parked sub_8025A64/sub_8025B0C/sub_8025BAC/sub_8025CA4 (NON_MATCHING C reconstructions widen this unit past its own real 0x08025D28 start) - stay raw, real bytes in asm/code_3_2_17_25a64.s - see docs/matching/issue-41-game-loop-25894.md
    (0x08025D74, None, "game_loop"),  # GitHub issue #41: sub_8025D74 - BG-scroll-layer hardware-register/bitfield initializer; left raw, real bytes in asm/code_3_2_17_25d74.s - see docs/matching/issue-41-game-loop-25894.md
    (0x08025DE8, "src/system/game_loop15.o", "game_loop"),  # GitHub issue #41: sub_8025DE8/sub_8025E2C (streamed-tile-range growers firing a self->0x30-table trampoline per step)/sub_8025E70/sub_8025E84 (their plain clamp-only counterparts); matched. Also incl. parked sub_8025D74 (NON_MATCHING C reconstruction widens this unit past its own real 0x08025DE8 start) - stays raw, real bytes in asm/code_3_2_17_25d74.s - see docs/matching/issue-41-game-loop-25894.md
    (0x08025E98, None, "game_loop"),  # GitHub issue #41: sub_8025E98 - computes the four screen-edge tile coordinates and drives the sub_8025E2C/sub_8025DE8 range growers plus two self->0x30-table trampolines; left raw, real bytes in asm/code_3_2_17_25e98.s - see docs/matching/issue-41-game-loop-25894.md
    (0x08025F24, "src/system/game_loop16.o", "game_loop"),  # GitHub issue #41: sub_8025F24 - truncates the Q8 position to a tile-scroll halfword pair and writes it through the self+0x58 hardware-register pointer; matched. Also incl. parked sub_8025E98 and sub_8025F3C (NON_MATCHING C reconstructions widen this unit both before its own real 0x08025F24 start and past its own real 0x08025F3C end) - both stay raw, real bytes in asm/code_3_2_17_25e98.s and asm/code_3_2_17_25f3c.s respectively - see docs/matching/issue-41-game-loop-25894.md
    (0x08025F3C, None, "game_loop"),  # GitHub issue #41: sub_8025F3C - circular-buffer decoded-tile streaming loop; left raw, real bytes in asm/code_3_2_17_25f3c.s - see docs/matching/issue-41-game-loop-25894.md
    ...
    (0x0803985C, None, "audio"),  # sub_803985C/sub_80398DC/sub_8039AA4/sub_8039B44/sub_8039E50/sub_8039F30 - more GAX2 channel/voice internals (sub_8039B44/sub_8039E50 are one logical routine split by a manual return-address trampoline, sub_8039FFC also could not have been matched around it) hitting the same many-register (r8/sb/sl) gcc-2.9 allocation difficulty documented for sub_8038538; left raw, issue #68
    ...
    (0x0803A03C, None, "audio"),  # sub_803A03C - hits the same many-register (r8) gcc-2.9 allocation difficulty; left raw, issue #68
    (0x0803A104, "src/audio/gax_channel_init.o", "audio"),  # sub_803A104 - per-channel voice object constructor/reset; matched, issue #68
    (0x0803A158, None, "audio"),  # sub_803A158 - hits the same many-register (r8/sb/sl) gcc-2.9 allocation difficulty; left raw, issue #68
    ...
    (0x0803A278, None, "audio"),  # sub_803A278/sub_803A2C8/sub_803A318/sub_803A324/sub_803A5A8/sub_803A608 - several more manual return-address-trampoline-linked function pairs plus the UnknownC type's play_fn (sub_803A324, many-register-heavy); left raw, issue #68. Also incl. the raw ARM-mode (not Thumb) DSP/mixer code block docs/audio.md documents (gStaticData_0803A630 onward) - not disassembled as code at all yet
    (0x0803A944, "src/system/timer_util.o", "system"),  # BIOS svc wrapper stubs (sub_803A944-sub_803A95C/sub_0803A960) + LZ77UnCompWrapper/RLUnCompWrapper (confirmed non-audio via matched asset_util.c callers) - NAKED but genuinely un-improvable trampolines with no real C logic to express, kept matched per docs/matching.md's frozen convention - plus sub_803A968 (picks a 12-byte EepromConfig table by chip-size code - see the struct's header comment) and sub_803A9D0 (claims a hardware timer, hands back an IRQ-handler-stub address); matched, issue #69
    (0x0803AA08, None, "system"),  # sub_803AA08 - arms the claimed timer - NAKED transcription, byte-correct but not real decompiled C, tracked as parked (a handful of narrow register-allocation gaps a plain-C reconstruction couldn't close) - see docs/matching/issue-69-eeprom-timer.md, issue #69
