...
UNITS = [
    ...
    (0x08008408, "src/graphics/actor_part6.o", "graphics"),  # sub_8008408/sub_8008434/sub_8008480/sub_8008484/sub_80084A4/sub_80084C4/sub_8008518 - a small gUnknown_03001308 sub-object accessor (same convention as sub_8007F78/sub_8006FE4), a part-object constructor, a trivial always-true stub, a table-swap-plus-conditional-sub_8026ED0 call (same shape as sub_80073BC in graphics.o), a part re-initializer (same sub_800725C/table-swap/sub_8007AB4 shape as sub_8008434, re-using an existing part instead of allocating one), and two sub_80083B8-derived keyframe-record pointer pickers (genuine native `switch`es - sub_8008518's case 0/4 grouping is non-adjacent enough on its own to force a jump table, while sub_80084C4's case labels needed scattering out of numeric order); non-adjacent to actor_part5.o since the parked sub_80083B8 sits raw between them; matched. New object, inserted between asm/code_3_2_6.o (ends after sub_80083B8's guard) and asm/code_3_2_7.o (starts at sub_8008564, the remainder of the old code_3_2_6.o content)
    (0x08008564, None, "game_loop"),  # remainder of the 94.4 KB main zone, still raw - docs/rom_map.md "Two big unnamed systems"
