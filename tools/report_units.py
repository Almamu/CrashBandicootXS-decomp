...
UNITS = [
    ...
    (0x0800A734, "src/graphics/actor_part48.o", "graphics"),  # sub_800A734/sub_800A810 (issue #9): part-object velocity/state reset+constructor pair - sub_800A734 additionally hooks up a child object at self+0xb0 via sub_800815C; sub_800A810 dispatches a sub-state byte (self+0x88) to one of three teardown helpers. PARKED (NON_MATCHING): field writes and dispatch semantics confirmed correct one-for-one, but the ROM's running-pointer address-increment style (sub_800A734) and exact compare/branch-chain shape plus self's register letter together (sub_800A810) weren't reproduced by any C phrasing tried - see docs/matching/issue-9-0x08007634-actor.md. Real bytes in the new asm/code_3_2_16_a734.s. New object, inserted between actor_part14.o and the trimmed asm/code_3_2_16.o (now starting at sub_800A884)
