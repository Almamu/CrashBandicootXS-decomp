...
UNITS = [
    ...
    (0x08008DC0, "src/graphics/actor_part11.o", "graphics"),  # sub_8008DC0/sub_8008DEC/sub_8008E50/sub_8008E94/sub_8008EB4/sub_8008EE4 - a table+0x20/0x24-trampoline-firing loop, two array-element removal functions sharing the same CpuSet-based compaction shape (one locating the index by a linear pointer-walk search, one taking the index directly as an argument), a capacity-checked array append, a two-array teardown (mem_free via sub_8026EB4, then optionally the manager itself via sub_8026ED0), and a two-array manager initializer (mem_alloc via sub_8026EC0, zero-filling the first array); non-adjacent to actor_part10.o since the parked sub_8008D80 sits raw between them; matched. New object, inserted between asm/code_3_2_12.o (now holding just the parked sub_8008D80) and the new asm/code_3_2_13.o (starts at sub_8008F20, the remainder of the old code_3_2_12.o content)
    (0x08008F20, None, "game_loop"),  # sub_8008F20 through sub_8009DF4-ish (~29 functions) - the remainder of a large, deeply interconnected AI/collision/physics dispatch system touching gUnknown_030012C0/gUnknown_030012D8, which docs/rom_map.md itself still describes as only partially understood; left raw rather than guess at semantics - docs/rom_map.md "Two big unnamed systems"
