...
UNITS = [
    ...
    (0x0800878C, "src/graphics/actor_part7.o", "graphics"),  # sub_800878C through sub_8008904 - two more sub_80083B8-derived keyframe-record byte lookups, a run of plain part+0x24/+0x28/+0x29/+0x2d/+0x30/+0x34/+0x38/+0x3c field accessors (some via the (u32<<N)>>31 logical-shift bit-getter idiom), a position-resolve-and-dispatch function (sub_8008890, forwarding to sub_8007634/sub_80073DC), and two more part-object table-swap constructors (sub_80088F0/sub_8008904, reusing sub_8008484/sub_80084A4); non-adjacent to actor_part6.o since the parked sub_8008770 sits raw between them; matched. New object, inserted between asm/code_3_2_7.o (now holding just the parked sub_8008770) and asm/code_3_2_8.o (starts at sub_800891C, the remainder of the old code_3_2_7.o content)
    (0x0800891C, None, "game_loop"),  # remainder of the 94.4 KB main zone, still raw - docs/rom_map.md "Two big unnamed systems"
