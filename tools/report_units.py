...
UNITS = [
    ...
    (0x08009FD4, None, "game_loop"),  # sub_8009FD4 itself, still raw (uncertain register roles for a 4th call argument to sub_803AD88) - docs/rom_map.md "Two big unnamed systems"
    (0x08009FF4, "src/graphics/actor_part9.o", "graphics"),  # sub_8009FF4 through sub_800A0F4 - an AABB-vs-region overlap test using two already-matched AABB builders (sub_8007C30/sub_8007CF8), another table+N-trampoline dispatcher (sub_800A050), a branchless `(-x|x)>>31` nonzero-test idiom (sub_800A06C), and a long run of plain self+0x44/+0x48/+0x4c/+0x50/+0x54/+0x58/+0x5c/+0x60/+0x64/+0x68/+0x69/+0x74 field accessors and bulk setters (the velocity/accel fields sub_8009DF4 clamps, among others); non-adjacent to actor_part8.o since the raw, unclaimed sub_8009FD4 sits between them. New object, inserted between asm/code_3_2_10.o (now holding just sub_8009FD4) and the new asm/code_3_2_11.o (starts at sub_800A0FC, the remainder of the old code_3_2_10.o content)
